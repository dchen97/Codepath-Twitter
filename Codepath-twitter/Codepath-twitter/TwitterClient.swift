...
class TwitterClient: BDBOAuth1SessionManager {
    ...
    func handleOpenUrl(url: URL) {
        ...
        fetchAccessToken(withPath: "https://api.tweitter.com/oauth/access_token", method: "POST", requestToken: requestToken, success: {(accessToken: BDBOAuth1Credential?) -> Void in
    ...
    func retweet(idString: String, success: @escaping () -> (), failure: @escaping(Error) -> ()) {
        post("1.1/statuses/retweet/\(idString).json", parameters: [], progress: nil, success: { (task:URLSessionDataTask, response: Any?) in
            success()
    ...
    func favorite(idString: String, success: @escaping () -> (), failure: @escaping(Error) -> ()) {
        post("1.1/favorites/create.json?id=\(idString)", parameters: [], progress: nil, success: {(task: URLSessionDataTask, response: Any?) in
                success()
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
